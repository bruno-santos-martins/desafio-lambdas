...
class InMemoryEmployeeRepository extends EmployeeRepository {
  ...
  async create(employeeData: Employee): Promise<Employee> {
    ...
    this.employeeStore.set(employeeData.id, JSON.parse(JSON.stringify(employeeData)));
    return employeeData;
  }
  async getById(employeeId: string): Promise<Employee | null> {
    return this.employeeStore.get(employeeId) || null;
  }
  async update(employeeId: string, updateFields: Partial<Employee>): Promise<Employee | null> {
    const existingEmployee = await this.getById(employeeId);
    if (!existingEmployee) return null;
    const updatedEmployee = { ...existingEmployee, ...updateFields } as Employee;
...
describe('Employee use-cases', () => {
  test('create and get employee', async () => {
    ...
    const createdEmployee = await createEmployee(employeeRepository, { nome: 'Ana', cargo: 'Dev', idade: 30 });
    expect(createdEmployee.id).toBeDefined();
    const fetchedEmployee = await getEmployee(employeeRepository, createdEmployee.id);
    expect(fetchedEmployee).toEqual(createdEmployee);
  });

  test('get employee by id', async () => {
    const employeeRepository = new InMemoryEmployeeRepository();
    const employeeData = { id: '123', nome: 'Carlos', cargo: 'Designer', idade: 28 } as Employee;
  ...
  test('update employee fields', async () => {
    const employeeRepository = new InMemoryEmployeeRepository();
    const createdEmployee = await createEmployee(employeeRepository, { nome: 'João', cargo: 'QA', idade: 25 });
    const updatedEmployee = await updateEmployee(employeeRepository, createdEmployee.id, { cargo: 'QA Sr', idade: 26 });
  ...
  test('delete employee', async () => {
    const employeeRepository = new InMemoryEmployeeRepository();
    const createdEmployee = await createEmployee(employeeRepository, { nome: 'Maria', cargo: 'PM', idade: 35 });
